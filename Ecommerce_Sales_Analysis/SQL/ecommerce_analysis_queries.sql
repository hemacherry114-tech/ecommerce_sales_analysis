...
UPDATE `ecommerce_sales`
SET
    `Order_Year` = YEAR(STR_TO_DATE(`ORDERDATE`, '%m-%d-%Y %H:%i')),
    `Order_Month` = MONTH(STR_TO_DATE(`ORDERDATE`, '%m-%d-%Y %H:%i')),
    `Order_Day_of_Week` = DAYOFWEEK(STR_TO_DATE(`ORDERDATE`, '%m-%d-%Y %H:%i'));
...
ORDER BY
    total_revenue DESC
LIMIT 5;
-- Analyze Sales by Country and Territory
SELECT
    `COUNTRY`,
    SUM(`Total_Revenue`) AS total_revenue
FROM
